      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 border-b border-gray-200">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-gray-600 mb-4">
              <div>
                ...
                <span className="text-base font-medium">{vehicle?.licensePlate}</span>
              </div>
              <div>
                <span className="font-semibold block">Marque/Modèle :</span> 
                <span className="text-base font-medium">{vehicle?.brand} {vehicle?.model}</span>
              </div>
              <div>
                <span className="font-semibold block">Année :</span> 
                <span className="text-base font-medium">{vehicle?.year}</span>
              </div>
              <div>
                <span className="font-semibold block">Agence :</span> 
                <span className="text-base font-medium">{vehicle?.base?.location}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {stats.attentePieces > 0 && (
                <Badge variant="secondary" className="text-sm bg-amber-100 text-amber-800 hover:bg-amber-100">
                  Attente pièces: {stats.attentePieces}
                </Badge>
              )}
              {stats.terminee > 0 && (
                <Badge variant="secondary" className="text-sm bg-green-100 text-green-800 hover:bg-green-100">
          ...
          <Badge variant="secondary" className="text-sm h-fit">
            {filteredInterventions.length} intervention{filteredInterventions.length > 1 ? 's' : ''} filtrée{filteredInterventions.length > 1 ? 's' : ''}
          </Badge>
        </div>
      </div>
        ...
        {filteredInterventions.length > 0 ? (
          <div className="space-y-4">
            {filteredInterventions.map(intervention => (
              <Card key={intervention.id} className="border-gray-200 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 text-sm text-gray-600">
                          {intervention.accordNumber && (
                            <div className="flex items-center gap-1">
                              <span className="font-medium">Accord :</span> {intervention.accordNumber}
                            </div>
                          )}
                          <div className="flex items-center gap-1">
                            <Calendar size={14} />
                            Début : {new Date(intervention.createdAt).toLocaleDateString('fr-FR')}
                          </div>
                          <div className="flex items-center gap-1">
                            <Clock size={14} />
                            Mis à jour : {new Date(intervention.statusUpdatedAt || intervention.createdAt).toLocaleDateString('fr-FR')}
                          </div>
                        </div>
                      </div>
